...
import {Check,CircleX,Crown,Goal,ShieldCheck,Trash2} from 'lucide-react';
...
export function GoalCenter({match,registrations}:{match:Match;registrations:Registration[]}){
 const {user}=useAuth();const {data:g}=useGroup();const qc=useQueryClient();const [scorer,setScorer]=useState('');const reportRequest=useRef(requestId());const directRequest=useRef(requestId());
 ...
 const {data}=useQuery({queryKey:key,queryFn:async()=>{
  ...
  if(error)throw error;if(openError)throw openError;return{events:(events||[]) as unknown as GoalEvent[],reportingOpen:Boolean(reportingOpen)};
 }});
 useRealtimeInvalidation(`goals-${match.id}`,['goal_events'],[key,['v2-stats',match.group_id],['player-card']],true);
 const refresh=()=>{qc.invalidateQueries({queryKey:key});qc.invalidateQueries({queryKey:['v2-stats',match.group_id]});qc.invalidateQueries({queryKey:['player-card']})};
 const participants=registrations.filter(r=>r.registration_status==='confirmed'&&r.attended);
 const scorerId=scorer||participants[0]?.user_id||'';const mine=participants.some(r=>r.user_id===user?.id);
 ...
 const events=data?.events||[];const approved=events.filter(e=>e.status==='approved');const pending=events.filter(e=>e.status==='pending');
 const scores=useMemo(()=>{const byPlayer=new Map<string,{userId:string;name:string;goals:number}>();approved.forEach(e=>{const current=byPlayer.get(e.scorer_user_id);if(current)current.goals++;else byPlayer.set(e.scorer_user_id,{userId:e.scorer_user_id,name:goalName(e.scorer),goals:1})});return[...byPlayer.values()].sort((a,b)=>b.goals-a.goals||a.name.localeCompare(b.name,'he'))},[approved]);
 const leaders=scores.length?scores.filter(x=>x.goals===scores[0].goals):[];
 return <Card className="space-y-4"><div className="section-title"><h2><Goal size={20}/>שערים במשחק</h2><Badge>{approved.length} שערים מאושרים</Badge></div>
  ...
  {(data?.reportingOpen&&mine||canManageGoals&&started)&&participants.length>0&&<div className="rounded-xl border border-white/10 p-3"><label className="mb-2 block text-sm font-extrabold">מי הבקיע?</label><div className="flex flex-col gap-2 sm:flex-row"><Select value={scorerId} onChange={e=>setScorer(e.target.value)}>{participants.map(p=><option key={p.user_id} value={p.user_id}>{fullName(p.profiles)}</option>)}</Select>{data?.reportingOpen&&mine&&<Button disabled={!scorerId||report.isPending||addDirect.isPending} onClick={()=>report.mutate()}><Goal size={17}/>דיווח גול</Button>}{canManageGoals&&started&&<Button variant="secondary" disabled={!scorerId||report.isPending||addDirect.isPending} onClick={()=>addDirect.mutate()}><ShieldCheck size={17}/>הוספה ישירה</Button>}</div>{data?.reportingOpen&&mine&&<p className="section-help mt-2">הדיווח ייכנס לסטטיסטיקה רק לאחר אישור מנהל המשחק.</p>}</div>}
  {started&&!participants.length&&<p className="empty-inline">יש לסמן נוכחות לפני שניתן לדווח על שער.</p>}
