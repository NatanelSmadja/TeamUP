...
type GoalScorer = {user_id: string; first_name: string; last_name: string; avatar_url?: string | null; goals: number | string};
  ...
  return <>
    ...
    <dialog ref={dialog} className="goal-scorers-dialog" aria-labelledby={titleId} dir="rtl" onClick={(event) => {
      ...
      <div className="leaderboard-table">{scorers.map((player, index) => <Link to={`/players/${player.user_id}`} key={player.user_id} className="leader-row" onClick={() => dialog.current?.close()}>
        <b>{index + 1}</b><PlayerAvatar profile={player} className="player-avatar"/><div><strong className={player.user_id === user?.id ? 'goal-scorer-me' : undefined}>{player.first_name} {player.last_name}{player.user_id === user?.id && ' (אני)'}</strong></div><div className="leader-stats"><span data-label="שערים">{player.goals}</span></div>
      </Link>)}</div>
