...
export default function Layout() {
  ...
  useEffect(() => {
    if (user?.id) void syncExistingPushSubscription().catch(() => undefined);
  }, [user?.id]);
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const requestedGroup = params.get('group');
    if (!requestedGroup || !memberships.length) return;
    ...
    <div className="app-shell">
      <aside className="desktop-sidebar">
        ...
        <div className="sidebar-profile">
          <NavLink to="/profile">
            <UserRound size={20} />
            <span>{profile?.first_name || 'פרופיל'}</span>
          </NavLink>
          <button onClick={signOut} title="יציאה מהחשבון">
            <LogOut size={19} />
          </button>
        </div>
      </aside>
      ...
      <div className="content-shell">
          ...
          <div>
            ...
            <div className="topbar-group-row">
              ...
              {memberships.length > 1 && (
                <select value={g?.group.id || ''} onChange={(e) => setActiveGroupId(e.target.value)}>
                  {memberships.map((x) => (
                    <option key={x.group.id} value={x.group.id}>
                      {x.group.name}
                    </option>
                  ))}
                </select>
        ...
        <header className="mobile-topbar safe-top">
          ...
          {groupOpen && memberships.length > 1 && (
            <div className="mobile-group-menu card">
              <div className="mobile-group-menu-head">
                <strong>מעבר קבוצה</strong>
                <NavLink to="/groups">ניהול קבוצות</NavLink>
              </div>
              {memberships.map((x) => (
                <button key={x.group.id} onClick={() => chooseGroup(x.group.id)} className={cn(x.group.id === g?.group.id && 'active')}>
                  <span className="group-color-dot" style={{background: x.group.theme_color || '#2563eb'}} />
                  <span>
                    <strong>{x.group.name}</strong>
                    <small>{x.member.role === 'admin' ? 'מנהל קבוצה' : 'שחקן'}</small>
                  </span>
                  {x.group.id === g?.group.id && <span className="active-check">פעילה</span>}
                </button>
              ))}
            </div>
          )}
        ...
        {canManage(g) && (
          <div className="quick-fab">
            <button title="פעולות מהירות" onClick={() => navigate('/admin')}>
              <Zap size={20} />
      ...
      <nav className="mobile-nav safe-bottom">
        {primaryMobile.map(([to, label, Icon]) => (
          <NavLink key={to} to={to} end={to === '/'} className={({isActive}) => cn('mobile-link', isActive && 'active')}>
            <Icon size={21} />
            <span>{label}</span>
          </NavLink>
        ))}
        <button className={cn('mobile-link mobile-more-button', moreIsActive && 'active')} onClick={() => setMoreOpen(true)}>
          <Menu size={22} />
          <span>עוד</span>
        </button>
      </nav>
      ...
      {moreOpen && (
        <div className="mobile-sheet-layer" role="dialog" aria-modal="true" aria-label="תפריט נוסף">
          ...
          <section className="mobile-more-sheet safe-bottom">
            ...
            <div className="mobile-sheet-head">
              ...
              <button onClick={() => setMoreOpen(false)} aria-label="סגירה">
                <X size={21} />
              </button>
            </div>
            ...
            <div className="mobile-more-grid">
              {moreMobile.map(([to, label, Icon]) => (
                <NavLink key={to} to={to} className={({isActive}) => cn('mobile-more-card', isActive && 'active')}>
                  <span className="mobile-more-icon">
                    <Icon size={23} />
                  </span>
                  <strong>{label}</strong>
                  <small>{mobileDescription(to)}</small>
                </NavLink>
              ))}
            </div>
          </section>
