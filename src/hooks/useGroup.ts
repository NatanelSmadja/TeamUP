...
export function useGroup(){
 ...
 const query=useQuery({
  ...
  queryFn:async()=>{
   ...
   return (data||[]).map((session:any)=>({member:session.member as Member,group:session.group as Group,permissions:(session.permissions||[]) as string[]})) as GroupSession[];
  }
 });
 ...
 useEffect(()=>{if(selected&&selected.group.id!==activeId){localStorage.setItem(storageKey,selected.group.id);setActiveId(selected.group.id)}if(selected&&selected.group.id===pendingId)localStorage.removeItem('teamup_pending_group');else if(!selected&&activeId){localStorage.removeItem(storageKey);setActiveId(null)}},[selected,activeId,pendingId,storageKey]);
 const setActiveGroupId=(id:string)=>{localStorage.setItem(storageKey,id);setActiveId(id);window.dispatchEvent(new Event(GROUP_CHANGE_EVENT))};
