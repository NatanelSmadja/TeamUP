...
export function useGroup() {
    ...
    const query = useQuery({
        ...
        queryFn: async () => {
            const {
                data: memberships,
                error
            } = await supabase.from('group_members').select('*, groups(*)').eq('user_id', user!.id).eq('status', 'active').order('joined_at');
            ...
            const {
                data: permissions,
                error: pe
            } = ids.length ? await supabase.from('member_permissions').select('group_member_id,permission_key').in('group_member_id', ids) : {
                data: [],
                error: null
            };
            if (pe) throw pe;
            return valid.map((membership: any) => ({
                member: membership as Member,
                group: membership.groups as Group,
                permissions: (permissions || []).filter((p: any) => p.group_member_id === membership.id).map((p: any) => p.permission_key)
            })) as GroupSession[];
        }
    });
    ...
    }, [selected, activeId, storageKey]);
    const setActiveGroupId = (id: string) => {
        if (!memberships.some(x => x.group.id === id)) return;
...
export function canManage(g: ReturnType<typeof useGroup>['data'], permission?: string) {
    ...
    if (!permission) return g.member.role === 'moderator' || g.permissions.length > 0;
    return g.member.role === 'moderator' || g.permissions.includes(permission)
}
