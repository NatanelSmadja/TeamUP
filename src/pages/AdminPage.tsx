...
export default function AdminPage() {
  ...
  const isMainAdmin = g?.member.role === 'admin' || g?.group.owner_id === g?.member.user_id;
  const canCreateMatch = canManage(g, 'create_match'),
    canCloseRegistration = canManage(g, 'close_registration'),
    canManageRegistrations = canManage(g, 'manage_registrations'),
  ...
  const {data: matches = []} = useQuery({
    ...
    queryFn: async () => {
      const {data, error} = await supabase.from('matches').select('*').eq('group_id', g!.group.id).order('match_date', {ascending: false}).order('start_time', {ascending: false});
  ...
  const {data: polls = []} = useQuery({
    ...
    enabled: !!g && canManagePolls,
    queryFn: async () => {
      const {data, error} = await supabase.from('weekly_polls').select('*,availability_votes(count),weekly_poll_responses(count)').eq('group_id', g!.group.id).order('week_start', {ascending: false}).order('created_at', {ascending: false});
      if (error) throw error;
      return data || [];
    },
  });
  const {data: members = []} = useQuery({
    ...
    queryFn: async () => {
      const {data, error} = await supabase.from('group_members').select('*,profiles(*),member_permissions(permission_key)').eq('group_id', g!.group.id).order('joined_at');
      if (error) throw error;
      return data as any[];
    },
  });
  useRealtimeInvalidation(
    `admin-${g?.group.id}`,
    ['matches', 'match_registrations', 'group_members', 'member_permissions', 'weekly_polls', 'availability_votes', 'weekly_poll_responses'],
    [
  ...
  const create = useMutation({
    mutationFn: async () => {
      const teamCount = Number(f.team_count),
        teamSize = Number(f.team_size),
  ...
  const editPoll = async (p: any) => {
    ...
    if (week === null) return;
    await updatePoll(p.id, {title: title.trim() || 'סקר זמינות', week_start: week}, 'הסקר עודכן');
  };
  ...
  const togglePerm = async (m: any, key: string, on: boolean) => {
    const {error} = await supabase.rpc('set_group_member_permission', {
      p_member_id: m.id,
      p_permission: key,
      p_enabled: on,
    });
    ...
    <div className="space-y-5">
      <div className="page-heading">
        ...
        <div className="segmented">
          ...
          {canManagePolls && (
            <button className={tab === 'polls' ? 'active' : ''} onClick={() => setTab('polls')}>
              סקרים
            </button>
          )}
          {canViewRatingAudit && (
          ...
          {open && canCreateMatch && (
            <Card className="form-card">
              ...
              <div className="form-grid">
                <div>
                  <FieldHelp title="שם המשחק">כותרת ייחודית תעזור להבדיל בין משחקים באותו שבוע.</FieldHelp>
              ...
              <div>
                ...
                <div className="form-grid triple">
                  <div>
                    <FieldHelp title="יעד נרשמים">מספר המקומות בהרשמה.</FieldHelp>
                    <Input type="number" value={f.capacity} onChange={(e) => setF({...f, capacity: e.target.value})} />
                  </div>
                  <div>
                    <FieldHelp title="מקסימום קבוצות">לפחות שתי קבוצות נדרשות למשחק.</FieldHelp>
                    <Input type="number" min={2} step={1} value={f.team_count} onChange={(e) => setF({...f, team_count: e.target.value})} />
                  </div>
                  <div>
                    <FieldHelp title="גודל קבוצה רצוי">משמש לחלוקה האוטומטית.</FieldHelp>
                    <Input type="number" value={f.team_size} onChange={(e) => setF({...f, team_size: e.target.value})} />
                  </div>
                </div>
              </div>
              ...
              <Card key={m.id}>
                <div className="admin-match-row">
                  ...
                  <div className="action-row">
                    ...
                    {m.status === 'registration_open' && canCloseRegistration && (
                      <>
                        <Button variant="secondary" onClick={() => rpc('set_match_registration', {p_match_id: m.id, p_open: false}, 'ההרשמה נסגרה')}>
                          <Lock size={17} />
                          סגירה
                        </Button>
                      </>
                    )}
                    {m.status === 'registration_closed' && canCloseRegistration && (
                      <Button variant="secondary" onClick={() => rpc('set_match_registration', {p_match_id: m.id, p_open: true}, 'ההרשמה נפתחה מחדש')}>
                        <Unlock size={17} />
                        פתיחה
                      </Button>
                    )}
                    ...
                    {isMainAdmin && (
                      <Button variant="danger" onClick={() => confirm('למחוק את המשחק וכל הנתונים שלו?') && rpc('delete_match', {p_match_id: m.id}, 'המשחק נמחק')}>
                        <Trash2 size={17} />
                      </Button>
                    )}
                  </div>
        ...
        <>
          ...
          {pollOpen && (
            <Card className="form-card">
              ...
              <div className="form-grid">
                <div>
                  <FieldHelp title="שם הסקר">לדוגמה: “משחק שלישי” או “משחק שישי בבוקר”.</FieldHelp>
                  <Input value={pollForm.title} onChange={(e) => setPollForm({...pollForm, title: e.target.value})} />
                </div>
                <div>
                  <FieldHelp title="השבוע שאליו הסקר שייך">ברירת המחדל היא השבוע הנוכחי.</FieldHelp>
                  <Input type="date" value={pollForm.week_start} onChange={(e) => setPollForm({...pollForm, week_start: e.target.value})} />
                </div>
                <div className="md:col-span-2">
                  <FieldHelp title="הסבר אופציונלי">מידע שהשחקנים צריכים לדעת.</FieldHelp>
            ...
            {polls.map((p: any) => {
              ...
              return (
                <Card key={p.id}>
                  <div className="admin-match-row">
                    <div>
                      <Badge>{isPollPast(p.week_start) ? 'ההצבעה עברה' : p.status === 'open' ? 'פתוח' : 'סגור'}</Badge>
                      <h2>{p.title || 'סקר זמינות'}</h2>
                      <p>
                        {formatDate(p.week_start)} · {voteCount} בחירות ימים · {unavailableCount} לא יכולים
                      </p>
                    </div>
                    <div className="action-row">
                      <Button variant="secondary" onClick={() => location.assign(`/availability?poll=${p.id}`)}>
                        צפייה
                      </Button>
                      ...
                      {p.status === 'open' ? (
                        ...
                        <Button variant="secondary" onClick={() => updatePoll(p.id, {status: 'open', closed_at: null}, 'הסקר נפתח מחדש')}>
                          <Unlock size={16} />
                          פתיחה
                        </Button>
                      )}
                      <Button variant="secondary" onClick={() => duplicatePoll(p)}>
                        <Copy size={16} />
                        שכפול
                      </Button>
      ...
      {tab === 'members' && canManageMembers && (
        <div className="member-admin-grid">
          {!isMainAdmin && (
            <Card>
              <p className="empty-inline">אפשר לנהל שחקנים ובקשות הצטרפות. שינוי הרשאות נשאר בידי מנהל הקבוצה.</p>
            </Card>
          )}
          {members.map((m: any) => (
            <Card key={m.id} className={m.status !== 'active' ? 'member-inactive' : ''}>
              ...
              {m.role !== 'admin' && (
                <>
                  {m.status === 'active' && isMainAdmin && (
                    <div className="permission-grid">
                      {perms.map(([key, label]) => {
                        const on = m.member_permissions?.some((p: any) => p.permission_key === key);
                        return (
                          <label key={key}>
                            <span>{label}</span>
                            <input type="checkbox" checked={on} onChange={(e) => togglePerm(m, key, e.target.checked)} />
                          </label>
                        );
                      })}
                    </div>
                  ...
                  <div className="action-row mt-4">
                    {m.status === 'active' ? (
                      <>
                        <Button variant="danger" onClick={() => confirm('להסיר את השחקן?') && rpc('remove_group_member', {p_member_id: m.id, p_permanent: false}, 'השחקן הוסר')}>
                          <UserMinus size={17} />
                          הסרה
                        </Button>
                        <Button variant="ghost" onClick={() => confirm('מחיקה קבועה. להמשיך?') && rpc('remove_group_member', {p_member_id: m.id, p_permanent: true}, 'המשתמש נמחק')}>
                          <Trash2 size={17} />
                          מחיקה קבועה
                        </Button>
                      </>
                    ) : (
                      <Button onClick={() => rpc('restore_group_member', {p_member_id: m.id}, 'השחקן הוחזר')}>
                        <UserPlus size={17} />
                        החזרה
                      </Button>
                    )}
