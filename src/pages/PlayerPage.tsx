...
import {ArrowRight,CalendarDays,Crown,Flame,Footprints,Goal,ImageDown,ShieldCheck,Star,Trophy} from 'lucide-react';
...
export default function PlayerPage(){
 ...
 const {data,isLoading}=useQuery({queryKey:['player-card',id,g?.group.id],enabled:!!id&&!!g,queryFn:async()=>{
  const [{data:p,error},{data:st,error:se},{data:trend,error:te},{data:goalRows,error:ge},{data:cleanSheetRows,error:ce}]=await Promise.all([
   ...
   supabase.rpc('get_player_clean_sheet_stats',{p_user_id:id,p_group_id:g!.group.id})
  ]);
  if(error)throw error;if(se)throw se;if(te)throw te;if(ge)throw ge;if(ce)throw ce;const goals=goalRows?.[0];const cleanSheets=cleanSheetRows?.[0];
  return{p,ratings:trend||[],mvp:Number(st?.mvp_count||0),games:Number(st?.games_count||0),avg:Number(st?.avg_rating??p.base_rating??3),ratingCount:Number(st?.rating_count||0),totalGoals:Number(goals?.total_goals||0),monthlyGoals:Number(goals?.monthly_goals||0),matchGoals:Number(goals?.current_match_goals||0),cleanSheets:Number(cleanSheets?.clean_sheets||0)};
 }});
 useRealtimeInvalidation(`player-stats-${id}-${g?.group.id}`,['goal_events','player_public_stats','player_ratings','mvp_votes','match_registrations','match_clean_sheet_events','matches'],[['player-card',id,g?.group.id]],!!id&&!!g);
