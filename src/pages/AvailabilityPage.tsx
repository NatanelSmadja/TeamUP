...
import {fullName,isPollDayPast,isPollPast} from '../lib/utils';
...
export default function AvailabilityPage(){
 ...
 const pollPassed=!!data&&isPollPast(data.poll.week_start);
 const toggle=useMutation({mutationFn:async({day,isUnavailable}:{day:number|null;isUnavailable:boolean})=>{
  ...
  if(pollPassed||(day!==null&&isPollDayPast(data.poll.week_start,day)))throw new Error('ההצבעה עברה');
 ...
 return <div className="space-y-5">
  ...
  {!!polls.length&&<Card className="poll-switcher"><div><small>הסקר שמוצג עכשיו</small><strong>{selectedPoll?.title||'סקר זמינות'}</strong></div><div className="poll-select-wrap"><ChevronDown size={17}/><select value={selectedPoll?.id||''} onChange={e=>setParams({poll:e.target.value})}>{polls.map((p:any)=><option key={p.id} value={p.id}>{p.title||'סקר זמינות'} · {formatWeek(p.week_start)} · {isPollPast(p.week_start)?'ההצבעה עברה':p.status==='open'?'פתוח':'סגור'}</option>)}</select></div></Card>}
  {isLoading&&<Card>טוען את ההצבעות...</Card>}
  {data&&<>
   <Card className="hero-card poll-summary-card"><div><p className="eyebrow">{pollPassed?'ההצבעה עברה':data.poll.status==='open'?'הצבעה פתוחה':'הסקר נסגר'}</p><h2>{data.poll.title||'סקר זמינות'}</h2><p>שבוע שמתחיל ב־{formatWeek(data.poll.week_start)}. כל סקר נשמר בנפרד, גם כשיש שני משחקים באותו שבוע.</p></div><div className="action-row"><Badge>{pollPassed?'ההצבעה עברה':data.poll.status==='open'?'פתוח':'סגור'}</Badge><Button variant="secondary" onClick={share}><Share2 size={17}/>שיתוף סיכום</Button></div></Card>
   ...
   {(data.poll.status!=='open'||pollPassed)&&<div className="privacy-banner"><Lock/><div><strong>{pollPassed?'ההצבעה עברה':'הסקר סגור'}</strong><p>{pollPassed?'אפשר לצפות בתוצאות, אך מועד ההצבעה כבר עבר.':'אפשר לצפות בתוצאות, אך לא לשנות הצבעה. מנהל יכול לפתוח אותו מחדש.'}</p></div></div>}
   <Card className={`unavailable-card ${unavailable?'selected':''}`}><button disabled={toggle.isPending||data.poll.status!=='open'||pollPassed} onClick={()=>toggle.mutate({day:null,isUnavailable:true})} className="poll-toggle" title="מסמן שענית לסקר אבל אינך יכול להגיע"><div className="poll-day-icon">{unavailable?<Check size={21}/>:<UserX size={21}/>}</div><div><small>תשובה לסקר</small><h2>{pollPassed?'ההצבעה עברה':'לא יכול'}</h2></div><Badge>{data.responses.length} שחקנים</Badge></button><div className="voter-list"><div className="voter-title"><Users size={16}/>מי לא יכול?</div>{data.responses.length?data.responses.map((r:any)=><span key={r.id} className="person-chip">{fullName(r.profiles)}</span>):<span className="empty-inline">אף אחד עדיין</span>}</div></Card>
   <div className="poll-grid">{dayNames.map((name,i)=>{const selected=mine.has(i);const voters=(data.votes||[]).filter((v:any)=>v.day_of_week===i);const dayPassed=isPollDayPast(data.poll.week_start,i);return <Card key={name} className={`poll-card ${selected?'selected':''}`}><button disabled={toggle.isPending||data.poll.status!=='open'||dayPassed} onClick={()=>toggle.mutate({day:i,isUnavailable:false})} className="poll-toggle"><div className="poll-day-icon">{selected?<Check size={21}/>:i+1}</div><div><small>{dayPassed?'ההצבעה עברה':'יום'}</small><h2>{name}</h2></div><Badge>{dayPassed?'ההצבעה עברה':`${voters.length} זמינים`}</Badge></button><div className="voter-list"><div className="voter-title"><Users size={16}/>מי הצביע?</div>{voters.length?voters.map((v:any)=><Tooltip key={v.id} label={`זמין ביום ${name}`}><span className="person-chip">{fullName(v.profiles)}</span></Tooltip>):<span className="empty-inline">עדיין אף אחד</span>}</div></Card>})}</div>
