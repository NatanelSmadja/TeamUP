...
export default function StatsPage(){
 ...
 const {data}=useQuery({queryKey:key,enabled:!!g,queryFn:async()=>{const month=new Date().toISOString().slice(0,10);const [{data:members,error},{data:stats,error:se},{data:monthly,error:me},{data:monthlyGoals,error:mge},{data:allGoals,error:age}]=await Promise.all([supabase.from('group_members').select('user_id,profiles(*)').eq('group_id',g!.group.id).eq('status','active'),supabase.from('player_public_stats').select('*').eq('group_id',g!.group.id),supabase.rpc('get_player_of_month',{p_group_id:g!.group.id,p_month:month}),supabase.rpc('get_group_goal_leaderboard',{p_group_id:g!.group.id,p_month:month}),supabase.rpc('get_group_goal_leaderboard',{p_group_id:g!.group.id,p_month:null})]);if(error)throw error;if(se)throw se;if(me)throw me;if(mge)throw mge;if(age)throw age;const map=new Map((stats||[]).map((x:any)=>[x.user_id,x]));const rows=(members||[]).map((m:any)=>{const st:any=map.get(m.user_id);return{id:m.user_id,profile:m.profiles,rating:Number(st?.avg_rating??m.profiles.base_rating??3),mvp:Number(st?.mvp_count||0),games:Number(st?.games_count||0)}}).sort((a,b)=>b.rating-a.rating);return{rows,monthly:monthly?.[0]||null,monthlyGoals:monthlyGoals||[],allGoals:allGoals||[]}}});
 useRealtimeInvalidation(`v2stats-${g?.group.id}`,['player_public_stats','match_registrations','player_ratings','mvp_votes','goal_events'],[key],!!g);
 const rows=data?.rows||[];const monthly:any=data?.monthly;const topGoals:any=data?.monthlyGoals?.[0];const cards=[['הדירוג הגבוה',rows.slice().sort((a,b)=>b.rating-a.rating)[0],Star],['מלך ה־MVP',rows.slice().sort((a,b)=>b.mvp-a.mvp)[0],Crown],['מלך ההופעות',rows.slice().sort((a,b)=>b.games-a.games)[0],Flame]] as const;
 ...
 {topGoals&&<Card className="player-of-month"><div className="month-crown"><Goal size={30}/></div><div><span><CalendarDays size={15}/>מלך השערים של {new Date().toLocaleDateString('he-IL',{month:'long'})}</span><h2>{topGoals.first_name} {topGoals.last_name}</h2><p>רק שערים שאושרו במשחקי החודש הנוכחי</p></div><div className="month-score"><strong>{topGoals.goals}</strong><span>שערים</span></div></Card>}
 ...
 <div className="grid gap-4 lg:grid-cols-2"><GoalBoard title={`שערי ${new Date().toLocaleDateString('he-IL',{month:'long'})}`} rows={data?.monthlyGoals||[]}/><GoalBoard title="שערים בכל הזמנים" rows={data?.allGoals||[]}/></div>
...
function GoalBoard({title,rows}:{title:string;rows:any[]}){return <Card><div className="section-title"><h2><Goal size={20}/>{title}</h2><Badge>{rows.length} מבקיעים</Badge></div><div className="leaderboard-table">{rows.map((p:any,i:number)=><Link to={`/players/${p.user_id}`} key={p.user_id} className="leader-row"><b>{i<3?['🥇','🥈','🥉'][i]:i+1}</b><div className="player-avatar">{p.first_name?.[0]||'ש'}</div><div><strong>{p.first_name} {p.last_name}</strong><span>שערים מאושרים בלבד</span></div><div className="leader-stats"><span><Goal size={14}/>{p.goals}</span></div></Link>)}</div>{!rows.length&&<p className="empty-inline">עדיין אין שערים מאושרים בתקופה זו.</p>}</Card>}
