...
import {Archive, Check, Clipboard, History, KeyRound, Palette, RefreshCw, Settings, ShieldCheck, Trash2, UserPlus, Users, X} from 'lucide-react';
    ...
    const {
        ...
        queryFn: async () => {
            const {data, error} = await supabase
                .from('group_members')
                .select('id,user_id,role,status,joined_at,profiles(first_name,last_name)')
            ...
            {canManageMembers && (
                <Card>
                    <div className="section-title">
                        ...
                        <Badge>{activeMembers.length} שחקנים פעילים</Badge>
                    </div>
                    ...
                    {membersLoading ? (
                        ...
                        <div className="member-admin-grid">
                            {members.map((member: any) => {
                                ...
                                return (
                                    <Card key={member.id} className={!isActive ? 'member-inactive' : ''}>
                                        <div className="member-head">
                                            <div className="player-avatar">{member.profiles?.first_name?.[0] || 'ש'}</div>
                                            <div>
                                                <h2>{name}</h2>
                                                <p>{isAdmin ? 'מנהל הקבוצה' : isActive ? 'שחקן פעיל' : 'שחקן בארכיון'}</p>
                                            </div>
                                        </div>
                                        {!isAdmin && (
                                                ...
                                                {isActive ? (
                                                    <>
                                                        <Button
                                                            ...
                                                            onClick={() =>
                                                                confirm(`להעביר את ${name} לארכיון?`) &&
                                                                updateMember.mutate({
                                                                    memberId: member.id,
                                                                    action: 'archive',
                                                                })
                                                        ...
                                                        <Button
                                                            ...
                                                            disabled={updateMember.isPending}
                                                            onClick={() =>
                                                                confirm(`להסיר את ${name} מהקבוצה לצמיתות? החשבון שלו וקבוצות אחרות לא ייפגעו.`) &&
                                                                updateMember.mutate({
                                                                    memberId: member.id,
                                                                    action: 'delete',
                                                                })
                            ...
                            {!members.length && <p className="empty-inline">אין שחקנים בקבוצה.</p>}
                        </div>
                    )}
                </Card>
            )}
