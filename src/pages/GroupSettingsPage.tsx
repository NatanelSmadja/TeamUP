...
export default function GroupSettingsPage() {
    ...
    const {data: requests = [], isLoading, error} = useQuery({
        ...
        queryFn: async () => {
            const {data, error} = await supabase.rpc('list_group_join_requests', {p_group_id: g!.group.id});
    ...
    const {data: audit = []} = useQuery({
        ...
        queryFn: async () => {
            const {
                data,
                error
            } = await supabase.from('audit_logs').select('*').eq('group_id', g!.group.id).order('created_at', {ascending: false}).limit(30);
            if (error) throw error;
            return data || []
        }
    });
    ...
    const review = async (id: string, approve: boolean) => {
        const {error} = await supabase.rpc('review_group_join_request', {p_request_id: id, p_approve: approve});
    ...
    const rotate = useMutation({
        mutationFn: async () => {
            const {data, error} = await supabase.rpc('rotate_group_invite_code', {p_group_id: g!.group.id});
            if (error) throw error;
    ...
    const archive = useMutation({
        mutationFn: async () => {
            const {error} = await supabase.rpc('archive_group', {p_group_id: g!.group.id, p_restore: false});
            if (error) throw error
        }, onSuccess: () => {
    ...
    return <div className="space-y-5">
        <div className="page-heading">
            <div><p>מועדון, הצטרפות, בעלות ובקרה</p><h1>הגדרות הקבוצה</h1></div>
            <Settings/></div>
        <Card className="form-card">
            <div><h2>זהות ומדיניות המועדון</h2><p>הפרטים האלה שולטים באופן שבו משתמשים מגלים ומצטרפים לקבוצה.</p></div>
            <div className="form-grid">
                <div><FieldHelp title="שם הקבוצה">אפשר לשנות בלי לפגוע בהיסטוריה.</FieldHelp><Input value={f.name}
                                                                                                    onChange={e => setF({
                                                                                                        ...f,
                                                                                                        name: e.target.value
                                                                                                    })}/></div>
                <div><FieldHelp title="מיקום קבוע">המגרש שבו משחקים בדרך כלל.</FieldHelp><Input
                    value={f.default_location} onChange={e => setF({...f, default_location: e.target.value})}/></div>
                <div><FieldHelp title="נראות">קבוצה פרטית אינה מופיעה בגילוי.</FieldHelp><Select value={f.visibility}
                                                                                                 onChange={e => setF({
                                                                                                     ...f,
                ...
                <div><FieldHelp title="מדיניות הצטרפות">אפשר להצטרף מיד, באישור או רק בהזמנה.</FieldHelp><Select
                    value={f.join_mode} onChange={e => setF({...f, join_mode: e.target.value})}>
                    <option value="open">פתוחה</option>
                    <option value="approval_required">באישור מנהל</option>
                    <option value="invite_only">הזמנה בלבד</option>
                </Select></div>
                <div className="md:col-span-2"><FieldHelp title="תיאור הקבוצה">מידע קצר לשחקנים.</FieldHelp><Input
                    value={f.description} onChange={e => setF({...f, description: e.target.value})}/></div>
        ...
        <Card className="form-card">
            <div className="space-y-2">
                ...
                <p className="text-sm leading-6 text-gray-600">
                    כאשר המעקב פעיל, סגירת סקר מעדכנת לכל שחקן רצף של סקרים שלא
                    נענו. היענות לסקר מאפסת את הרצף.
                </p>
            </div>
      ...
      <span className="font-medium">
        הפעלת מעקב והתראה למנהלים
                ...
                <div className="space-y-2">
                    <FieldHelp title="התראה אחרי כמה סקרים שלא נענו">
                        המנהל יקבל התראה ויחליט אם להסיר את השחקן. ניתן לבחור בין 1
                        ל־20.
                    </FieldHelp>
                    <Input
                        type="number"
        ...
        {owner && <Card>
            ...
            <div className="section-title"><h2><KeyRound/>קישור הזמנה</h2><Badge>לבעלים בלבד</Badge></div>
            <p className="empty-inline">הקישור מצרף שחקנים ישירות לקבוצה. החלפת הקוד מבטלת מיד קישורים ישנים.</p>
            <div className="action-row"><Input readOnly value={inviteUrl}/><Button variant="secondary" onClick={() => {
                navigator.clipboard?.writeText(inviteUrl);
                toast.success('הקישור הועתק')
            }}><Clipboard size={16}/>העתקה</Button><Button variant="secondary"
                                                           onClick={() => rotate.mutate()}><RefreshCw size={16}/>החלפת
                קוד</Button></div>
        </Card>}

        {canManageMembers && <Card>
            <div className="section-title"><h2><UserPlus/>בקשות הצטרפות</h2><Badge>{requests.length}</Badge></div>
            {isLoading ? <p className="empty-inline">טוען בקשות...</p> : error ?
                <p className="empty-inline">לא הצלחנו לטעון: {error instanceof Error ? error.message : 'שגיאה'}</p> :
                <div className="join-request-list">{requests.map(r => <div key={r.request_id}>
                    <div className="player-avatar">{r.first_name?.[0] || 'ש'}</div>
                    <div><strong>{fullName({
                        first_name: r.first_name,
                        last_name: r.last_name
                    } as any)}</strong><span>{new Date(r.created_at).toLocaleDateString('he-IL')}</span></div>
        ...
        {owner && <Card>
            ...
            <div className="form-grid">
                <div><FieldHelp title="העברת בעלות">הבעלים החדש חייב להיות חבר פעיל בקבוצה.</FieldHelp><Select
                    defaultValue=""
            ...
            <Button variant="danger"
                    onClick={() => confirm('הקבוצה תועבר לארכיון ותוסתר מהחברים. להמשיך?') && archive.mutate()}><Archive
                size={16}/>ארכיון קבוצה</Button></Card>}
        {canManageMembers && <Card>
            <div className="section-title"><h2><History/>Audit Log</h2><Badge>{audit.length}</Badge></div>
            <div className="audit-list">{audit.map((a: any) => <div key={a.id}>
                <strong>{a.action}</strong><span>{new Date(a.created_at).toLocaleString('he-IL')}</span><small>{a.entity_type}</small>
            </div>)}{!audit.length && <p className="empty-inline">עדיין אין פעולות מתועדות.</p>}</div>
        </Card>}
    </div>;
}
